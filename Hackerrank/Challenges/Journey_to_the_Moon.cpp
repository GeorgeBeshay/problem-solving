...
long long journeyToMoon(int n, vector<vector<int>> astronaut) {
    ...
    for (const auto& edge: astronaut) {
        edges[edge[0]].push_back(edge[1]);
        edges[edge[1]].push_back(edge[0]);
    }

    vector<long long> graphSizes;
    ...
    for (int node = 0; node < n; node++) {
        ...
        long long tempGraphSize = 0;
        queue<int> nodesToVisit;
        nodesToVisit.push(node);

        while (!nodesToVisit.empty()) {

            int tempNode = nodesToVisit.front();
            nodesToVisit.pop();
            ...
            vector<int> neighborNodes = edges[tempNode];
            for (int neighborNode: neighborNodes) {
        ...
        graphSizes.push_back(tempGraphSize);
    }

    long long ans = 0;
    long long tempSum = 0;
    for (long long graphSize : graphSizes) {
        ans += tempSum * graphSize;
        tempSum += graphSize;
    }
